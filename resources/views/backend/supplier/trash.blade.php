        ...
        <table id="myDataTable" class="table table-bordered display">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a class='btn btn-sm btn-warning' href="{{route('backend.supplier.restore', $supplier->id)}}">Restore</a>
                        <form action="{{ route('backend.supplier.pdelete', $supplier->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button  id="bttn" class="btn btn-danger btn-sm" onclick="confirmDelete('{{ route('backend.supplier.pdelete', $supplier->id) }}')">Parmanent Delete</button>
                        </form>
