...
<div class="card">
    <div class="card-header">
        <a href="{{route('backend.account.index')}}" class="btn btn-primary">Account index</a>
    ...
    <div class="card-body">
        <table id="myDataTable" class="table table-bordered display">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$i++}}</td>
                    <td>{{$category->employee_id}}</td>
                    <td>{{$category->name}}</td>
                    <td>{{$category->role_name}}</td>
                    <td>{{$category->date}}</td>
                    <td>{{$category->start_cash_amount}}</td>
                    <td>{{$category->end_cash_amount}}</td>
                    <td>{{$category->sales_amount}}</td>
                    <td>
                        <a class='btn btn-sm btn-warning' href="{{route('backend.account.restore',$account->id)}}">Edit</a>
                        <form action="{{ route('backend.account.pdelete', $account->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button  id="bttn" class="btn btn-danger btn-sm" onclick="confirmDelete('{{ route('backend.account.pdelete', $account->id) }}')">Parmanently Delete</button>
                        </form>
