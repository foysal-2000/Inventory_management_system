...
<div class="card">
    <div class="card-header">
        <a href="{{route('backend.admin.index')}}" class="btn btn-warning">index</a>
        ...
        <table id="myDataTable" class="table table-bordered display">
            ...
            @foreach($users as $user)
                <tr>
                    ...
                    <td>
                        <a href="{{route('backend.admin.restore',$user->id)}}" class='btn btn-primary'>Restore</a>
                        <form action="{{ route('backend.admin.pdelete', $user->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button  id="bttn" class="btn btn-danger btn-sm" onclick="confirmDelete('{{ route('backend.admin.pdelete', $user->id) }}')">Parmanently Delete</button>
                        </form>
                    </td>
                </tr>
            @endforeach 
